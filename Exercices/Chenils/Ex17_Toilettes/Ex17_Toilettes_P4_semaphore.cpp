...
Implementation: with semahores only.
WITH MUTEX TRANSMISSION
*/
...
class ConcreteNSemToilet : public AbstractToilet {
/*
...
// Wait condition
QSemaphore* womanWaiting, manWaiting;
...
public:
ConcreteNSemToilet(int nbSeats, int nbMaxPeopleInRow) : AbstractToilet(nbSeats), nbMaxPeopleInRow(nbMaxPeopleInRow) {
	womanWaiting = new QSemaphore(0);
	manWaiting = new QSemaphore(0);
...
virtual void womanAccessing() {
	...
	mutex->acquire();
	if (nbManInside != 0 || nbWomanInside >= nbSeats || currentNbPeopleInRow >= nbMaxPeopleInRow) {
		nbWomanWaiting++;
		mutex->release();
		womanWaiting->acquire(); // mutex reception
	}
	
	nbWomanInside++;
	currentNbPeopleInRow++;

	if (nbWomanInside < nbSeats && currentNbPeopleInRow < nbMaxPeopleInRow) {
		nbWomanWaiting--;
		womanWaiting->release(); // mutex transmission
	} else {
		mutex->release();
	}
}
...
virtual void manAccessing() {
	mutex->acquire();
	if (nbWomanInside != 0 || nbManInside >= nbSeats || currentNbPeopleInRow >= nbMaxPeopleInRow) {
		nbManWaiting++;
		mutex->release();
		manWaiting->acquire(); // mutex reception
	}
	
	nbManInside++;
	currentNbPeopleInRow++;

	if (nbManInside < nbSeats && currentNbPeopleInRow < nbMaxPeopleInRow) {
		nbManWaiting--;
		manWaiting->release(); // mutex transmission
	} else {
		mutex->release();
	}
}
